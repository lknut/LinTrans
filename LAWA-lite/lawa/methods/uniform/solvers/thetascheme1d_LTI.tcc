...
{   
	...
	//std::cout << "thetascheme1d_LTI.assembletol = " << assembletol << std::endl;
	...
	// flens::DenseVector<flens::Array<T> > lhs = rhsmatrix * u_init;// + rhsvector;
	
	// for (int i=0; i< lhs.length(); i++){
	// 	std::cout << " thethascheme1d_LTI: lhs("
	// 			  << i << ") = " << lhs(i) << std::endl;
	// }
    ...
    if (use_pcg) {
		...
		//std::cout << "thetascheme1d_LTI.pgmresm call"  << std::endl;
        int iters = pgmresm(P,lhsmatrix, u, rhs, lintol,10);//10);
        //std::cerr << "Solve by gmres with iters = " << iters<< std::endl;
        //pgmresm(P,lhsmatrix, u, rhs, lintol, 20);
    }
	//std::cout << " u = " << u << std::endl;
	// flens::DenseVector<flens::Array<T> > tmp = lhsmatrix * u;
	// for (int i=0; i< tmp.length(); i++){
	//  	std::cout << " thethascheme1d_LTI: tmp(" << i << ") = " << tmp(i) << std::endl;
	// }
	// for (int i=0; i< u_init.length(); i++){
	// 	std::cout << " thethascheme1d_LTI: u(" << i << ") = " << u(i) << std::endl; 
	// }
