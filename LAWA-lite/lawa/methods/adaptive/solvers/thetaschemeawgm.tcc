namespace lawa {
  ...
  : timestep_solver(_timestep_solver), theta(1.), timestep(0.1), numOfTimesteps(10), timestep_eps(0.1),
													  ...
													  int &avDof, int &maxDof, int &terminalDof, int j)
	{
		...
		//LK: removed std::stringstream filename;
		//LK: removed filename << "conv_awgm_in_thetascheme_" << u.size() << ".txt";
		//LK: removed std::ofstream file(filename.str().c_str());
		...
		for (int i=1; i<=numOfTimesteps+2; ++i) {
			...
			std::cerr << "   Max level = (" << jmax_x << ", " << jmax_y << ")" << std::endl;
	...
	//LK: u_nextToLast saves the next to last result (that can be used to approximate sensitivites wrt t)
	template <typename Index, typename ThetaTimeStepSolver>
	void
	ThetaSchemeAWGM<Index,ThetaTimeStepSolver>::solveWithNextToLast(Coefficients<Lexicographical,T,Index> &u,Coefficients<Lexicographical,T,Index> &u_nextToLast,
													  int &avDof, int &maxDof, int &terminalDof, int j)
	{
		size_t hms;
		hms = u.bucket_count();
		...
		for (int i=1; i<=numOfTimesteps+2; ++i) {
			...
			//LK, save next to last solution
			if ((i== numOfTimesteps -1) ) u_nextToLast=u;
