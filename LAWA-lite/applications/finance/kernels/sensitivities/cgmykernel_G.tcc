...
template <typename T>
Kernel<T,CGMYeSensitivitiesG>::Kernel(const ProcessParameters1D<T,CGMY> &_params)
...
template <typename T>
Kernel<T,CGMYeSensitivitiesG>::Kernel(const ProcessParameters1D<T,CGMYe> &_params)
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::TailIntegralMoments(T x, int l) const {
    if (l == 0) {
        assert(fabs(x)>0);
        if (x>0) return 0.0; //LK, was  (C/Y)*( (std::pow(x,-Y)*exp(-M*x)) * (1+(M/(1-Y))*x)
        ...
        if (x>0) return 0.0; //LK, was C/(1-Y)*( -std::pow(x,1-Y)*exp(-M*x)
        ...
        if (x>0) return 0.0; //LK, was  C*std::pow(M,Y-l)*boost::math::tgamma(l-Y,M*x);
        else     return -  C*std::pow(G,Y-l)*boost::math::tgamma(l-Y,-G*x); // was +
    }
    else exit(1);
}
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::FirstTailIntegral(T x) const {
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::SecondTailIntegral(T x) const{
    //return this->nthTailIntegral(x, 2, ZeroAtInfinity);
    assert(fabs(x)>0);
	//LK, tmp!!!!!	
    if (x>0) return  0.0;/*-(C/(Y*(1-Y)))*(  boost::math::tgamma(2-Y,M*x)*(
...
//LK, changed
 template <typename T>
 T
 Kernel<T,CGMYeSensitivitiesG>::ThirdTailIntegral(T x) const {
     //return this->nthTailIntegral(x, 3, ZeroAtInfinity);
     assert(fabs(x)>0);
     if (x>0) return 0.0; //LK,w as C/(Y*(1-Y))*( - boost::math::tgamma(2-Y,M*x)*( 2*std::pow(M,Y-1)*x
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::ForthTailIntegral(T x) const {
    ...
    if (x>0) {
        ...
        return 0.0; //LK, was -PowX_3mY*ExpMMX*( OnedivSix*CdivY*(1 + Mdiv1mY*x) + 0.5*Cdiv1mY )
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::FifthTailIntegral(T x) const {
	...
	if (x>0) {
		...
		return 0.0; /*PowX_4mY*ExpMMX*( CdivY*(1 + Mdiv1mY*x)/24 + Cdiv1mY/6 )
...
template <typename T>
T
Kernel<T,CGMYeSensitivitiesG>::SixthTailIntegral(T x) const {
