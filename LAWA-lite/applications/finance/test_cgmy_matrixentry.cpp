...
int main(int argc, char *argv[])
{
	...
	// LK: all different combinations of wavelets, whereas $\Psi_0 = \Phi_1$ is the MRA-part
	for (int k_row=basis.mra.rangeI(j0).firstIndex(); k_row<=basis.mra.rangeI(j0).lastIndex(); ++k_row) { // LK: geht MRA j0 durch, k_row
		...
		j1 = j0; k1 = k_row; xtype1 = XBSpline; // LK: xtype1
		for (int k_col=basis.mra.rangeI(j0).firstIndex(); k_col<=basis.mra.rangeI(j0).lastIndex(); ++k_col) { // LK: k_col
			if (   (d==3 && k_col<=basis.mra.rangeIL(j0).lastIndex())
			       || (d==3 && k_col>=basis.mra.rangeIR(j0).firstIndex()) ) continue;
			j2 = j0; k2 = k_col; xtype2 = XBSpline; //LK: xtype2
			...
			T approxVal = singularIntegral(j1,k1,xtype1,1, j2,k2,xtype2,1); // LK: rechnet auf (a_1,a_2) \times (b_1,b_2)...
		...
		for (int j_col=j0; j_col<J; ++j_col) { // LK: j_col
			for (int k_col=basis.rangeJ(j_col).firstIndex(); k_col<=basis.rangeJ(j_col).lastIndex(); ++k_col) {
				...
				j2 = j_col; k2 = k_col; xtype2 = XWavelet; // LK: xtype2
	...
	for (int j_row=j0; j_row<J; ++j_row) { // LK: j_row
		for (int k_row=basis.rangeJ(j_row).firstIndex(); k_row<=basis.rangeJ(j_row).lastIndex(); ++k_row) { // LK: k_row
			j1 = j_row; k1 = k_row; xtype1 = XWavelet; // LK: xtype1
