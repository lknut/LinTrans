...
//////////////////////////////
// LK: LinearTransformation //
//////////////////////////////


template <typename Basis2D>
FinanceOperator2D<CGMYeLinearTransformation2DJump2D, Basis2D>::FinanceOperator2D
(const Basis2D& _basis, const ProcessParameters2D<T,CGMYeLinearTransformation2DJump2D> &_processparameters,
 T _R1_1, T _R2_1, T _R1_2, T _R2_2, int order,
 int _internal_compression_level1, int _internal_compression_level2)
: basis(_basis), processparameters(_processparameters),
  R1_1(_R1_1), R2_1(_R2_1), R1_2(_R1_2), R2_2(_R2_2),
  internal_compression_level1(_internal_compression_level1),
  internal_compression_level2(_internal_compression_level2),                                            // predef_conv,predef_reaction
  cgmyeop1d_1(basis.first, processparameters.proc_param1,R1_1,R2_1,order,internal_compression_level1,0.,0.,false,true), //LK: zero convection term
  cgmyeop1d_2(basis.second,processparameters.proc_param2,R1_2,R2_2,order,internal_compression_level2,0.,0.,false,true),
  integral1(basis.first,basis.first), integral2(basis.second,basis.second)
{
	//LK, convection part.
}
...
template <typename Basis2D>
void
FinanceOperator2D<CGMYeLinearTransformation2DJump2D, Basis2D>::setCompressionLevel(int _internal_compression_level1,
                                                                       int _internal_compression_level2)
{
    internal_compression_level1 = _internal_compression_level1;
    cgmyeop1d_1.setCompressionLevel(internal_compression_level1);
...
template <typename Basis2D>
typename Basis2D::T
FinanceOperator2D<CGMYeLinearTransformation2DJump2D, Basis2D>::operator()(const Index2D &row,
                                                              const Index2D &col)
{
    T ret = 0.;
    // evaluate x1-direction for multiwavelet discretization -> if x2 indices are not identical, the
...
template <typename Basis2D>
void
FinanceOperator2D<CGMYeLinearTransformation2DJump2D, Basis2D>::eval
(Coefficients<Lexicographical,T,Index2D> &v, Coefficients<Lexicographical,T,Index2D> &Av,
 const char* evalType)
{
    typedef typename Coefficients<Lexicographical,T,Index2D>::const_iterator            const_coeff2d_it;
...
template <typename Basis2D>
void
FinanceOperator2D<CGMYeLinearTransformation2DJump2D, Basis2D>::performMV
(Coefficients<Lexicographical,T,Index2D> &v, Coefficients<Lexicographical,T,Index2D> &Av)
{
    typedef typename Coefficients<Lexicographical,T,Index2D>::const_iterator            const_coeff2d_it;
    typedef typename Coefficients<Lexicographical,T,Index2D>::iterator                  coeff2d_it;
