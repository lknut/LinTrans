...
FinanceOperator1D<T, CGMYeSensitivitiesr, Basis1D>::FinanceOperator1D
      ...
      //data(196613)
{
  ...
  if (use_predef_convection) {
    ...
    //convection = 0.;
	//LK this is printed, so there really is convection! -> predef_confection
...
template <typename T, typename Basis1D>
void
FinanceOperator1D<T, CGMYeSensitivitiesr, Basis1D>::setCompressionLevel(int _internal_compression_level)
{
	// internal_compression_level = _internal_compression_level;
}
...
template <typename T, typename Basis1D>
T
FinanceOperator1D<T, CGMYeSensitivitiesr, Basis1D>::operator()(XType xtype1, int j1, int k1,
                                                 XType xtype2, int j2, int k2) const
{
	//LK derivative of the bilinearform wrt r is zero!
	return (T)0.0;
	...
	// // // LK, use_predef_reaction=true running cgmy_....cpp
	...
	// //LK: mu, lambda vertauscht?
	// T part1=0., part2=0.;
	// if (basis.d==2) {

	// 	if (j1>=11 && j2>=11) {
	// 		//pde_val=0.134358891945* OneDivR2pR1 * integral(j1,k1,xtype1,0,j2,k2,xtype2,1);
	// 		int_val = -singularIntegral(j1,k1,xtype1,1, j2,k2,xtype2,1);

	// 		//ERROR FOR DIFFERENT CALCULATION -> TODO

			
	// 			//std::cout << " cgmyeoperator1d_G.tcc int_val = " << int_val << " pde_val = " << pde_val << std::endl;
	// 	 }
	// 	 else {
	...
	// 	  		//LK, part1 = c_3 * \varphi_{\lambda}(x_i)
	...
	// 	  		}
	// 	  	}
	// 	 	int_val = part1 + part2; //LK, this value equals (B.17)

	// 		//T int_val2=int_val;
	// 		//std::cout << "intval, fast = " << int_val<< std::endl;
	// 		//int_val = -singularIntegral(j1,k1,xtype1,1, j2,k2,xtype2,1);			
	// 		//std::cout << "intval, fast - slow = " << int_val - int_val2<< std::endl;
	// 		//std::cout << kernel.ForthTailIntegral(-0.00000000001) - kernel.constants[3] << std::endl;*/
	// 	 }
...
template <typename T, typename Basis1D>
T
FinanceOperator1D<T, CGMYeSensitivitiesr, Basis1D>::operator()(const Index1D &row_index,
