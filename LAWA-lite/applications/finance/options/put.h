...
struct Option1D<T,Put>
{
	...
	Option1D( const Option1D<T,Put>& _option );

	
	Option1D<T,Put>
	operator= (Option1D<T,Put> const& rhs);

	void
	swap(Option1D<T,Put> & rhs);
	...
	//LK, derivative of the payoff wrt G
	T
	payoff_log(T x, ProcessType1D _processtype) const;
	...
	//LK this function has to be called in order to use the linear transformation model,
	// it initializes the w
	void
	calc_w( const  ProcessParameters1D<T,CGMYe>  &_processparamters);

    OptionParameters1D<T,Put> optionparameters;
    DenseVector<Array<T> >    singularPoints;
    std::map<T,T> values;
	T wr; //LK (w+r)
	T dwdG; //derivative of w wrt G
	T dwdM; //derivative of w wrt G
	T dwdr; //derivative of w wrt G
