...
struct TruncatedPutOption1D
{
	...
	//LK, new
	//static ProcessType1D processtype;
	static ProcessType1D processtype;
	...
	//LK: this has to be used in order to use the linear-transformation-Model
	//static void
    //setOption(const Option1D<T,Put> &_option,const  ProcessParameters1D<T,CGMYe>  &_processparamters);
	...
	static T
    g_trunc(T x, ProcessType1D _processtype);

	static void
	setProcesstype(ProcessType1D _processtype1D);

	static T
	g_trunc_derivative(T x);

	

	//static T
	//log_payoff_G_trunc(T x);
};
